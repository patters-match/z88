...
public class Z88Keyboard {
	 ...
	 *	-------------------------------------------------------------------------
	 *	A15 (#7) | RSH    SQR     ESC     INDEX   CAPS    :       $       ^
	 *	A14 (#6) | HELP   LSH     TAB     DIA     MENU    ;       M       �
	 *	A13 (#5) | *      SPACE   &       A       Q       W       L       �
	 *	A12 (#4) | =      LFT     �       Z       S       X       ,       P
	 *	A11 (#3) | )      RGT     "       E       D       C       K       �
	 *	A10 (#2) | -      DWN     '       R       F       V       J       O
	 *	A9  (#1) | <      UP      (       T       G       B       U       I
	 *	A8  (#0) | DEL    ENTER   �       Y       H       N       �       !
	 *	-------------------------------------------------------------------------
	 * </PRE>
	 *
	 */
	private Map createFrLayout() {
		...
		addKey(keyboardLayout, (0x10000 | 0xF9), 0x06FE);	// '�'		
		addKey(keyboardLayout, (0x10000 | '%'), 0x06FE);	// SHIFT '�' = '%'
		// --------------------------------------------------------------------------------------------------------------------------
	...
	private Map createDkLayout() {
		...
		// Single key:
		addKey(keyboardLayout, KeyEvent.VK_COMMA, 0x06FB);
		addKey(keyboardLayout, (0x10000 | 0xE6), 0x06FD); // '�'
		addKey(keyboardLayout, (0x10000 | 0xC6), 0x06FD); // '�'
		addKey(keyboardLayout, (0x10000 | 134), 0x06FD); // CTRL �
		addKey(keyboardLayout, (0x10000 | 0xF8), 0x06FE); // '�'
		addKey(keyboardLayout, (0x10000 | 0xD8), 0x06FE); // '�'
		addKey(keyboardLayout, (0x10000 | 152), 0x06FE); // CTRL �
		// --------------------------------------------------------------------------------------------------------------------------
		...
		// A13 (#5) | �      SPACE   1       Q       A       Z       L       0
		// Single key:
		addKey(keyboardLayout, (0x10000 | 0xE5), 0x057F); // '�'
		addKey(keyboardLayout, (0x10000 | 0xC5), 0x057F); // '�'
		addKey(keyboardLayout, (0x10000 | 133), 0x057F); // CTRL �
		...
		// A9  (#1) | /      UP      5       T       G       B       U       I
		// Single key:
		// '/' does not exist as a single key press, so we steel the '^' key next to '�' key.
	...
	private Map createSeFiLayout() {
		...
		// Single key:
		addKey(keyboardLayout, KeyEvent.VK_COMMA, 0x06FB);
		addKey(keyboardLayout, (0x10000 | 0xF6), 0x06FD); // '�'
		addKey(keyboardLayout, (0x10000 | 0xD6), 0x06FD); // '�'
		addKey(keyboardLayout, (0x10000 | 150), 0x06FD); // CTRL �
		addKey(keyboardLayout, (0x10000 | 0xE4), 0x06FE); // '�'
		addKey(keyboardLayout, (0x10000 | 0xC4), 0x06FE); // '�'
		addKey(keyboardLayout, (0x10000 | 132), 0x06FE); // CTRL �
		// --------------------------------------------------------------------------------------------------------------------------
						...
						if (currentKbLayout == z88Keyboards[COUNTRY_FR]) {
							switch(e.getKeyCode()) {
								...
								case KeyEvent.VK_3:  // PC [ALT Gr '3'] = '#' converts to Z88 <>" = #
									...
									break;
								case KeyEvent.VK_4:  // PC [ALT Gr '4'] = '{' converts to Z88 <>� = {
									releaseZ88key(z88SquareKey); // only DIAMOND (is already) pressed...
									kp = getZ88Key(KeyEvent.VK_7);
									break;
								case KeyEvent.VK_5:  // PC [ALT Gr '5'] = '[' converts to Z88 <>� = [
									releaseZ88key(z88SquareKey); // only DIAMOND (is already) pressed...
								...
								case KeyEvent.VK_9:  // PC [ALT Gr '9'] = '^' converts to Z88 <>� = ^
									...
									kp = getZ88Key(KeyEvent.VK_6);
									break;
								case KeyEvent.VK_0:  // PC [ALT Gr '0'] = '@' converts to Z88 <>� = @
									releaseZ88key(z88SquareKey); // only DIAMOND (is already) pressed...
									kp = getZ88Key(KeyEvent.VK_2);
									break;
								case KeyEvent.VK_RIGHT_PARENTHESIS:  // PC [ALT Gr ')'] = ']' converts to Z88 <>� = ]
									releaseZ88key(z88SquareKey); // only DIAMOND (is already) pressed...
