...
import com.jira.cambridgez88.ozvm.*;
...
public class FileArea {
    ...
    /**
     * Constructor<br> Scan file area at specified slot and populate this object
     * with information.
     *
     * @param slotNo
    ...
    /**
     * Dump the old linked list of File Entries and re-scan the current file
     * area to build a new linked list of File entries.
     */
    ...
    /**
     * If files exist in file area, a ListIterator is returned, otherwise null.
     * <p>Use next() on the iterator to get a
     * net.sourceforge.z88.filecard.FileEntry object that contains all available
     * information about each file entry.</p>
     *
    ...
    /**
     * Get an array of Strings that contains all active file names in the file
     * area. Used typically for JList Swing widget.
     *
    ...
    /**
     * Find file entry by filename and return a reference to the found object,
     * or return null if the file entry wasn't found in the file area.
     *
    ...
    /**
     * Return the amount of deleted file space in File Area, ie. the amount of
     * bytes occupied in the File Area the are used by files that are marked as
     * deleted.
     *
    ...
    /**
     * Get the total number of active files in the file area, ie. only those
     ...
     *
     * @param slotNo of File Card/Area
     * @param fileName the filename for the file to be stored (in "oz" filename
     * format).
     * @param fileImage the byte image of the file to be stored.
     *
    ...
    public void storeFile(String fileName, byte[] fileImage)
        ...
        if (isFileAreaAvailable() == false) {
            ...
            if ((1 + fileName.length() + 4 + fileImage.length) > getFreeSpace()) // not enough free space for file entry header and file image
            {
    ...
    public boolean isImportable(File selectedFiles[])
        ...
        if (isFileAreaAvailable() == false) {
            ...
            if (totalFileSize <= getFreeSpace()) {
                return true;
            } else // not enough free space for selected files
            {
                return true;
            }
        }
    ...
    /**
     * Import a file from the Host file system into the Z88 File Area. The
     * Filename of the host file system will be converted into the filenaming
     * format of the Z88 File Card. Due to limits of the Z88 filename format,
     * the host filename might get truncated.
     *
     * @param hostFile
     * @throws FileAreaNotFoundException
    ...
    public void importHostFile(File hostFile)
        ...
        if (isFileAreaAvailable() == false) {
            ...
            if (hostFile.isFile() == false) {
                ...
                if ((1 + z88FileName.length() + 4 + hostFile.length()) > getFreeSpace()) // not enough free space for file entry header and file image
                {
                    throw new FileAreaExhaustedException();
                } else {
    ...
    /**
     * Export a file entry on the file area to the host filing system. The path
     * of file entry is truncated, and only the core filename is appended to the
     * specified host filing system path.
     *
    ...
    /**
     * Import all files from the Host directory into the Z88 File Area. The
     * filenames of the host file system will be converted into the filenaming
     * format of the Z88 File Card. Due to limits of the Z88 filename format,
     * the host filename might get truncated.
     *
     * @param hostDirectory
        ...
        if (isFileAreaAvailable() == false) {
            ...
            if (hostDirectory.isDirectory() == false) {
                ...
                for (int f = 0; f < fileList.length; f++) {
                    if (fileList[f].isFile() == true) {
                        importHostFile(fileList[f]);
                    }
                }
            }
        }
     ...
     * been removed/replaced with another card), and update the linked list of
     * file entries, if a new File area has become available.
     *
    ...
    /**
     * Evaluate whether a file area can be created on the card in the specified
     * slot. Ram card and empty slots will automatically be discarded (returns
     * false). If empty space is found on card (ie. below an application area)
     * and that it conforms to the rules of minimum needed file space for flash
     * cards (64K, except AMD 128K chip that has 16K sectors) and UV Eprom (16K)
     * then true is returned.
     *
     * Use the create() method for the specified slot to actually create/format
     * the file area.
     *
     * This method also returns true if an existing file area is found (which
     * means that the file area could be re-formatted).
     *
     * @param slotNumber
     * @return true if a file area might be created on the card, otherwise
     * return false
     */
            ...
            if (fileHdrBank != -1) {
                ...
                if (slotinfo.isApplicationCard(slotNumber) == true) {
                    ...
                    if (bank instanceof EpromBank == true) {
                        if (memory.getExternalCardSize(slotNumber) == appCrdHdr.getAppAreaSize()) {
     ...
     *
     * The complete file area will be formatted with FFh's from the bottom of
     * the card up until the File Area header.
     *
     * @param slotNumber
    ...
    /**
     * Validate that the file image is in fact a copy of a file card: identified
     * with an 'oz' watermark at the top of the card and make sure that the file
     * size is matches a known card size, eg. 32K, 128K .. 1024K.
     *
     * @param fileEprImage
    ...
    public static boolean checkFileAreaImage(File fileEprImage) {
        ...
        try {
            ...
            // get bank size byte
            f.seek(f.length() - 4);
            if (f.readByte() * 16384 != f.length()) // total number of banks doesn't match file image size...
            {
                fileAreaStatus = false;
            }
    ...
    public static boolean createFileHeader(final int bankNo) {
        ...
        if ((bank instanceof EpromBank == true)
            ...
            if ((bank instanceof EpromBank == true)) {
                if (memory.getExternalCardSize(slotNo) == 2) // a 32K Eprom was identified...
                {
    ...
    /**
     * Mark file entry as deleted, using specified filename (in "oz" filename
     * format).
     *
     * @param fileName
    ...
    /**
     * Format file area with FF's, beginning from top of specified bank (or bank
     * of file header) downwards until bottom of (type of) card has been
     * reached.
     *
     * @param topBank the top bank of the file area including the header at
     * $3FC0
     */
    ...
    /**
     * Convert the extended address for the current slot to a calculable
     * integer.
     *
    ...
    /**
     * Reclaim deleted file space in file area, ie. get a copy of all the active
     * files, then reformat the file area (all traces of deleted files are gone)
     * and finally save the active files back to the file area.
     *
     * @throws FileAreaNotFoundException
     */
    ...
    private class FileEntryCache extends FileEntry {
        ...
        /**
         * Constructor.<br> Create a cached FileEntry that also preserves the
         * file image
         */
