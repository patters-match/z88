 ...
 * package also adds support for embedded textual comments. <p> There are still
 * some limitations. For instance, animations are limited to a single global
 * color table. But that is usually what you want anyway, so as to avoid
 * irregularities on some displays. (So this is not really a limitation, but a
 * "disciplinary feature" :) Another rather more serious restriction is that the
 * total number of RGB colors in a given input-batch mustn't exceed 256.
 * Obviously, there is an opening here for someone who would like to add a
 * color-reducing preprocessor. <p> The encoder, though very usable in its
 * present form, is at bottom only a partial implementation skewed toward my own
 * particular needs. Hence a couple of caveats are in order. (1) During
 * development it was in the back of my mind that an encoder object should be
 * reusable - i.e., you should be able to make multiple calls to encode() on the
 * same object, with or without intervening frame additions or changes to
 * options. But I haven't reviewed the code with such usage in mind, much less
 * tested it, so it's likely I overlooked something. (2) The encoder classes
 * aren't thread safe, so use caution in a context where access is shared by
 * multiple threads. (Better yet, finish the library and re-release it :) <p>
...
public class Gif89Encoder {
     ...
     * @param ci_pixels Array of color-index pixels no less than width * height
     * in length.
     * @exception IOException See the addFrame() methods.
     */
     ...
     * @exception IOException If Gif89Frame can't be accommodated. This could
     * happen if either (1) the aggregate cross-frame RGB color count exceeds
     * 256, or (2) the Gif89Frame subclass is incompatible with the present
     * encoder object.
     */
     ...
     * aggregate cross-frame RGB color count exceeds 256, or (3) this encoder
     ...
     * IndexGif89Frame <i>per se</i>. But I might add some pedantic check later,
     * to justify the generality :)
     */
     ...
     * if any. A value of -1 turns off transparency. (Default: -1)
     */
    ...
    /**
     * Write the Gif frame directly to the [out] stream, avoiding the internal
     * Vector storage (a large, or long animation with hundreds of frames will
     * properly not fit within the Java Heap!).
     *
     * When the last frame has been written, use:
     * <pre>
     ...
     * @throws IOException
     */
    public void encodeFrame(OutputStream out, Gif89Frame gifFrame) throws IOException {
        if (getFrameCount() < 1) {
            addFrame(gifFrame);
        } else {
            if (gifHdrWritten == false) {
            ...
            accommodateFrame(gifFrame);
            gifFrame.encode(out, true, colorTable.getDepth(), colorTable.getTransparent());
        }
    }
