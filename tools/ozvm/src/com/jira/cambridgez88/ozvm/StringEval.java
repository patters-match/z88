...
public class StringEval {
	...
	private static boolean isBinaryDigits(final String strSequence) {
		for(int d=0, n=strSequence.length(); d<n; d++) {
			if (strSequence.charAt(d) != '0' & strSequence.charAt(d) != '1') {
				if (d != n-1) 
					// a digit was found in the 'middle' that wasn't a '0' or '1'
					return false;
				else
					// the last binary digit may be a 'b' specifier..
					if (strSequence.charAt(n-1) != 'b') return false;
			}
		}
		...
		for(int d=0, n=strSeq.length(); d<n; d++) {
			if (Character.isDigit(strSeq.charAt(d)) == false) {
				if ("abcdef".indexOf(strSeq.charAt(d)) < 0) {
					...
					if (d != n-1) 
						// a digit was found in the 'middle' that wasn't 
						// in the '0' - '9' range
						return false;
					else
						// the last binary digit may be a 'h' specifier..
						if (strSeq.charAt(n-1) != 'h') return false;
				}
			}
		}
		...
		for(int d=0, n=strSequence.length(); d<n; d++) {
			if (Character.isDigit(strSequence.charAt(d)) == false) {
				if (d != n-1) 
					...
					// the last binary digit may be a 'd' specifier..
					if (strSequence.charAt(n-1) != 'd') return false;
			}
		}
	...
	/**
	 * Coerce string to 16bit integer.<br>
	 * The string is scanned to determine the number format, but might be pre-determined
	 * with a trailing type specifier; 'd' for decimal, 'h' for hexadecimal or 'b' for binary.
	 * When not specifying a type, ambiguity may happen where it cannot be determined 
	 * whether a string is a decimal, hexadecimal or a binary. In those cases hexadecimal 
	 * is used as default. 
	 * 
	 * @param strNumber
	 * @return converted integer, or -1 if there was a syntax error in the number format or range
	 */
