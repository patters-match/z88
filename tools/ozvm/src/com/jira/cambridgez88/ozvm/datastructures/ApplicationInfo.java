...
/**
 * Get Access to Z88 Application Information (DOR) for available slot. Also,
 * validate Application Card Image.
 */
public class ApplicationInfo {
    ...
    /**
     * Scan slots 0-3 for installed applications to update the information
     * previously gathered by this class instance.
     */
    ...
    /**
     * If applications exist in card at specified slot, a ListIterator is
     * returned, otherwise null. <p>Use next() on the iterator to get an
     * ApplicationDor object that contains all available information about the
     ...
     * Identified with an 'OZ' watermark at the top of the card and a bank
     * counter less or equal to the size of the image.
     *
     * @param applImage
    ...
    public static boolean checkAppImage(File applImage) {
        ...
        try {
            ...
            if ((f.length() > 1024 * 1024) | (f.length() % 16384 != 0)) // illegal card size
            {
            ...
            // get bank size byte
            f.seek(f.length() - 4);
            if (f.readByte() * 16384 > f.length()) // total number of banks larger than image size...
            {
                fileImageStatus = false;
            }
