...
;
; --------------------------------------------------------------------------------------------------------------------
; OZ Shell Z80 assember toolchain suite - zln, links *.obj to *.bin
; --------------------------------------------------------------------------------------------------------------------
;
...
include "zlncmd.inc"                            ; zELF shell command definition and constants
INCLUDE "rtmvars.def"
...
xref initvars,initpointers, close_files
xref initfiles
...
.valoption
        ...
        jr      z, flag_loop
        cp      'm'
        jr      z, setopt_mapfile               ; generate map file
...
.disp_usage
        oz      OS_Pout
        defm    "usage: zln ",1,"2+T[-m] {file.obj ...} | @project",1,"2-T",CR,LF,0
        cp      a
        jp      exit_zlncmd

.setopt_mapfile
        set     opt_mapfile,(iy + options)    ; -m
        ret


; ******************************************************************************
; Before any error message, report "zln: " to console
.err_Hdr
        oz      OS_Pout
        defm    "z80: ",0
        ret
