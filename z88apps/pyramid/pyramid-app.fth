...
\ Notes about converting this program to be a Z88 application are
\ made throughout the text, highlighted with "**Z88 Application**"
...
\ **Z88 Application**
\ All static data and definitions will be in ROM2, together with names
\ Any variables defined with VARIABLE will automatically be allocated
\ dataspace in RAM, so we only need to worry about CREATEd words and
\ VALUEs.
...
\ **Z88 Application**
\ VOCABULARY is provided, so we don't need this
...
\ **Z88 Application**
\ All these CREATEd tables contain static, initialised data and so should
\ be in ROM, except "obj-locs" which contains dynamic, uninitialised data
\ and so should be in RAM.
...
\ **Z88 Application**
\ The values and defining words (using CREATE) throughout this program
\ are only used when actually compiling it, and not during execution.
\ Therefore, they should all be left in ROM.
...
\ **Z88 Application**
\ We'll now add a higher level definition to call GAME and CATCH any
\ possible errors. This will be our entry point from COLD.
\ To test that unexpected errors are correctly trapped, you can
\ alter the word to turn on ESCape detection (5 OS_ESC DROP) and press
\ ESC during execution.
...
' PYRAMID IS (COLD)                  \ This is our entry word from COLD

\ **Z88 Application**
\ Now we'll add a word to deal with special command codes. We'll define
\ two later in the application DOR; Quit and Restart, which will have
\ command codes 128 and 129 respectively.

: COMMAND-KEYS  ( eventcode -- )
    256 -                            \ Convert event code to command code
    CASE  128 OF  BYE  ENDOF         \ for the QUIT command (code 128)
          129 OF  COLD  ENDOF        \ for the RESTART command (code 129)
          7 EMIT                     \ Just beep for other events
    ENDCASE ;

' COMMAND-KEYS IS (ACC_EVT)          \ Use as (ACC_EVT)
' COMMAND-KEYS IS (KEY)		     \ and as (KEY)


\ **Z88 Application**
\ Finally, we need to think about dealing with the standard error codes.
\ For RC_ESC and RC_QUIT, the default vectors (ESCAPE and BYE) are fine.
\ For RC_DRAW, we can also redescribe the current location. We'll still
\ ask OZ to preserve screen when possible, so this won't be called very
\ often. To avoid having to do this, we'd need to keep more careful track
\ of what state the screen was in, in order to redraw it correctly.

: PYRAMID-DRAW
    CINIT                            \ re-draw the console window
    game-over @                      \ check if a game is in progress
    IF  ." Another go (Y/N)? "       \ if not, we must be at this point
    ELSE  describe                   \ if game running, describe location
    THEN ;

' PYRAMID-DRAW IS (RC_DRAW)          \ Use as the action for RC_DRAW errors
...
\ **Z88 Application**
\ We'll now set the namespace and dataspace back to RAM, so that
\ the DOR creation tools can be loaded in RAM.
